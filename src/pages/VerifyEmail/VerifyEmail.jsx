...
const VerifyEmail = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.email) {
      ...
      setError(
        t('auth.emailNotFound', { defaultValue: 'Email not found. Please try signing up again.' }),
      );
    }
  }, [location, t]);
      ...
      setError(
        err.message ||
          t('auth.verificationFailed', {
            defaultValue: 'Verification failed. Please check the code.',
          }),
      );
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      <div className={s.content}>
        <p className={s.instruction}>{t('auth.enterOtp')}</p>
        ...
        <form onSubmit={handleVerify}>
          ...
          <Button type="submit" loading={loading} className={s.verifyBtn}>
            {t('auth.verifyAndSignIn')}
          </Button>
        </form>

        {successMessage && <div className={s.successToast}>{successMessage}</div>}
      </div>
    </AuthLayout>
  );
};
